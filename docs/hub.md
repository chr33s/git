...
For that reason a projection MUST carry **more than one** checkpoint — the newest few, bounded — and a verifier walks them newest-first to the first credible one. Keeping only the greatest `issuedAt` turns the rule above into a freeze: one attestation dated ahead, from a malicious admin or a box with a fast clock, becomes the only checkpoint on record, and refusing it refuses every write on a repository with `maxTrustAgeSeconds` set — the write to `refs/meta/policy` that would lift the bound included. The fold stays clockless; the clock lives in the verifier, which is the only place that has one.

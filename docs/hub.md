...
A remote that is replicated to has to authenticate, and a stored token cannot: every credential a hub-enabled destination accepts is either a signature over the request in hand or a delegation capped at the §12 maximum, so a token written into a registry authenticates until tomorrow and then stops. Behind a standing instruction that is invisible — the forward runs detached and reports into a log, so the mirror goes quiet, revocations included, while the origin goes on accepting the pushes it is failing to send. Therefore a registry MUST be able to hold a **signing key** rather than a token, and a credential MUST be minted from it where it is used rather than read out of storage. The minted credential is bound to this repository's `RepoID`, which is the right one exactly when the destination is a replica of this repository — which is what replication is; against anything else it presents as garbage, as a credential minted for the wrong repository always has. It claims the whole capability vocabulary short of `repo.admin`, because a mirror carries whatever landed and each namespace is charged a different capability at the far boundary; claiming is not holding, since verification intersects the claim with what the issuer holds, so it authorizes exactly what the key's owner could have pushed by hand. A key is write-only in the registry for the same reason a token is. Registering a _delegated_ credential against a standing instruction MUST be refused where it is written, naming the key that does keep working — a manual remote may keep one, because what comes back when it stops working comes back to whoever asked.
...
Every writer of an append-only ref MUST take its compare-and-swap value from what the ref _holds_, not from what it resolves to: the same value is the parent of the record being written and the expected value of the swap that publishes it, and the store compares the swap against the ref's own contents. Read through a symbolic ref the two differ, and every append then conflicts against a value nobody wrote — on a namespace with no way back, spending the retry that exists for concurrent authors on a conflict no retry can clear. Nothing makes these refs symbolic today; the writers must not be the reason it stays that way.

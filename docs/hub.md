...
Every writer of an append-only ref MUST take its compare-and-swap value from what the ref *holds*, not from what it resolves to: the same value is the parent of the record being written and the expected value of the swap that publishes it, and the store compares the swap against the ref's own contents. Read through a symbolic ref the two differ, and every append then conflicts against a value nobody wrote — on a namespace with no way back, spending the retry that exists for concurrent authors on a conflict no retry can clear. Nothing makes these refs symbolic today; the writers must not be the reason it stays that way.

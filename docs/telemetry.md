...
Git+ telemetry records the observable runtime conditions surrounding agent work without turning Git into a general observability backend.
...
Internally, pre-call Context Exposure and post-call runtime telemetry remain separate signed records because they occur at different times and have different failure modes. The server/UI joins them into one Invocation projection.
...
Each canonical trace record is carried by a signed Git record commit.
...
For example, an Invocation Telemetry record references the prior Context Exposure:
...
OTel `TraceId`/`SpanId`, provider request IDs, conversation IDs, and harness event IDs are correlation metadata, not durable Git+ record identity.
...
Trace history may be a DAG. Implementations MUST preserve causal parentage and MUST NOT infer causation solely from timestamps.
...
> **OTel defines what the incoming signal means; Git+ defines how selected audit facts are durably represented.**
...
If no convention version/revision is known, Git+ MAY use a documented best-effort mapping but MUST NOT claim strict semconv adherence for that signal.

### 4.2 Stable Git+ schema

The durable schema remains independent of evolving upstream attribute names:
...
Transport through OTel does not upgrade evidence class.
...
Products MUST NOT silently promote reported or derived values to observed facts.
...
Automatic transient retries occurring inside that span MUST NOT be split into invented logical invocations.

### 6.2 Provider attempts are subordinate

If instrumentation separately exposes provider-attempt detail, Git+ MAY attach it beneath the logical Invocation:
...
If no attempt-level instrumentation exists, the product reports attempts as unknown/omitted rather than guessing.

### 6.3 Fallbacks follow upstream boundaries

If provider/model fallback is represented by another inference span, Git+ creates another logical Invocation. If upstream keeps it inside one logical span, Git+ keeps one Invocation and MAY attach observed attempt/fallback detail.
...
Requested and response model identifiers remain distinct. If only one is known, omit the other rather than copying it.
...
### 7.3 Outcome versus finish reason

Operation outcome and generation finish are distinct:
...
A successful generation ending because of a length limit is not automatically an error.

Git+ MUST NOT collapse finish reasons, OTel span status, and `error.type` into one field.
...
`conversation.externalId` is correlation metadata and MUST NOT replace authoritative Git+ session identity.
...
Raw tool arguments/results are frequently sensitive and SHOULD NOT be canonical by default.
...
It MUST NOT be treated as proof that retrieved material crossed the invocation boundary. Repository Context Pack / Context Exposure remain authoritative for that question.
...
The record SHOULD omit unknown values rather than inventing zeros/defaults.
...
If the denominator is unknown or semantically incompatible, omit the ratio.
...
A separate `context-compaction` trace record SHOULD exist only when the harness directly observes a transition with additional facts such as strategy/reason.

Likewise, truncation events should be emitted only when directly observable rather than inferred from absent content.

---

## 11. Workspace transitions

Git tree identity remains authoritative for repository workspace state.

A useful boundary-driven capture pattern is:
...
A workspace transition SHOULD record only what is needed for audit, for example:
...
A deployment claiming durable per-Invocation audit SHOULD therefore use a dedicated path:
...
OTel Baggage MUST NOT be used as an integrity or authority channel for:
...
If batched records are used, each entry requires an immutable locator such as:
...
The browser/CLI receives a projected Invocation rather than reconstructing protocol state itself.
...
When attempts are not explicitly observed, omit the Attempts section.
...
A branch lookup remains useful:

```bash
git+ session show --branch=<branch> --audit
```

The Flight Recorder presents Invocation rows and preserves concurrent trace lanes when the history is a DAG.
...
Native OTel ingestion should normally make direct use of the same underlying trace writer without requiring a shell command per span/event.

No normal `git+ trace show` workflow is required; `session show --audit` is the product-level projection.
...
- Weak/partial runtime capture does not invalidate otherwise valid Git repository evidence.
...
7. provider usage remains reported rather than observed;
...
11. known sampling/transformation/loss weakens coverage claims;
12. Git record OIDs remain canonical audit identity;
13. high-volume trace data never enters policy-critical session folds;
14. the normal product UX presents one Invocation rather than requiring users to understand separate pre/post trace records;
15. `session show --audit` and the Flight Recorder preserve causal DAG structure;
16. external OTel retention is not required to inspect the durable Git+ audit history.

---

## Final invariant

> **OpenTelemetry GenAI is the preferred runtime semantic input; Git+ is the durable signed audit projection. Context Exposure and runtime telemetry remain separate records internally but project as one Invocation for users. Upstream span meaning survives normalization, Git OIDs remain canonical identity, loss is surfaced rather than hidden, and detailed observability never becomes policy-path cost.**

...
describe("Artifacts local provider", () => {
  ...
  it.effect("revokes a deleted repository's tokens, so the name can be reused safely", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
          const client = yield* bound;
  ...
  it.effect("issues, lists, revokes tokens — and raw fails as a typed error", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
          const client = yield* bound;
  ...
  it.effect("refuses a delete whose name is not a repository name", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
          const client = yield* bound;
  ...
  it.effect("refuses to delete a repository its forks read through", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
          const client = yield* bound;
  ...
  it.effect("refuses in memory too to drop a repository something reads through", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
  ...
  it.effect("re-forks in memory without emptying the child or stranding its own forks", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
          const repoStores = yield* RepoStores;
  ...
  it.effect("forks over alternates: shared history, isolated writes", () =>
    Effect.promise(async () => {
      await run(
        Effect.gen(function* () {
          const client = yield* bound;
          ...
          const childOid = yield* Effect.gen(function* () {
            ...
            const tree = yield* repository.writeTree([
              { mode: "100644", name: "f.txt", oid: blob },
            ]);
  ...
  it.effect("imports a repository over smart HTTP from the node host", () =>
    Effect.promise(async () => {
...
describe("the node provider's fork links", () => {
  ...
  it.effect("refuses a fork of a parent whose storage is gone", () =>
    Effect.promise(async () => {
  ...
  it.effect("hands a re-forked child back from its old parent", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
  ...
  it.effect("keeps every link when forks are made at once", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
        await seed(root, "parent");
  ...
  it.effect("takes a deleted name out of the lists that lend to it", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
        await seed(root, "parent");
        await Effect.runPromise(stores.fork("child", "parent"));
  ...
  it.effect("rebuilds a fork of a fork when the middle one is re-forked", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
  ...
  it.effect("goes looking only for a name whose storage is already gone", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
  ...
  it.effect("goes looking when the lender its borrower names cannot be named back", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
  ...
  it.effect("unlends a parent its borrower named through a symlink", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
        await seed(root, "parent");
        await seed(root, "child");
  ...
  it.effect("refuses to drop a repository something still borrows", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
        await seed(root, "parent");
        await Effect.runPromise(stores.fork("child", "parent"));
  ...
  it.effect("counts a borrower that has no fork link, as `gc` does", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
        await seed(root, "parent");
  ...
  it.effect("does not write down which way round it broke a cycle", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "artifacts-links-"));
      try {
        for (const name of ["a", "b"]) await seed(root, name);
  ...
  it.effect("forgets a name's alternates as well as its link", () =>
    Effect.promise(async () => {
      await withRoot(async (root, stores) => {
        await seed(root, "parent");
        await Effect.runPromise(stores.fork("child", "parent"));
  ...
  it.effect("picks up a grandparent restored after the fork was opened without it", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "artifacts-links-"));
      try {
        // SAFETY: `repoStoresNode` discharges the only requirement; the cast

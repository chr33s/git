...
describe("cli hub", () => {
  ...
  it.effect("gives a repository an identity, then grants and lists membership", () =>
  ...
  it.effect("revokes a member, and says so in the listing", () =>
    Effect.promise(async () => {
      await cli(["init", "--root", root, "project"]);
  ...
  it.effect("does not call a re-instated member revoked on the strength of an old window", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses a subject that is not a fingerprint, and says what one looks like", () =>
    Effect.promise(async () => {
      await cli(["init", "--root", root, "project"]);
      await writeKeyPair(path.join(root, "id_root"), "root@example.com");
      await cli(["hub", "init", "--root", root, "--key", path.join(root, "id_root"), "project"]);
  ...
  it.effect("leaves the root key able to push, not merely able to grant", () =>
    Effect.promise(async () => {
  ...
  it.effect("fails rather than reporting success for a capability the fold refuses", () =>
    Effect.promise(async () => {
  ...
  it.effect("administers a repository whose threshold needs two signatures", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses a threshold its own signers cannot meet", () =>
    Effect.promise(async () => {
  ...
  it.effect("says what to do when a repository has no genesis yet", () =>
    Effect.promise(async () => {
  ...
  it.effect("mints a credential for a member of a hub-enabled repository", () =>
    Effect.promise(async () => {
      await cli(["init", "--root", root, "project"]);
      await writeKeyPair(path.join(root, "id_root"), "root@example.com");
      await cli(["hub", "init", "--root", root, "--key", path.join(root, "id_root"), "project"]);
  ...
  it.effect("lets a member bulk-commit through a host that guards the repository", () =>
    Effect.promise(async () => {
  ...
  it.effect("stops synchronizing hub state without touching the source", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to disable a directory that is not the clone the url was pinned as", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to disable a repository it was never enabled against", () =>
    Effect.promise(async () => {
  ...
  it.effect("declines trust on first use when stdin has already ended", () =>
    Effect.promise(async () => {
  ...
  it.effect("will not pin an identity whose own roots never signed it", () =>
    Effect.promise(async () => {
  ...
  it.effect(
    "does not charge a merge into its own side a force-push for spelling it as an oid",
    () =>
      Effect.promise(async () => {
        ...
        try {
          ...
          await call("branches/create", owner.credential, {
            name: "side",
  ...
  it.effect("refuses a push before it unpacks it, on a delete-only credential", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to write objects on a delete-only credential", () =>
    Effect.promise(async () => {
  ...
  it.effect("does not charge a rebase onto a branch it is creating a force-push", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to negotiate with a remote on a push-only credential", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to register a webhook on a push-only credential", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to send a repository's objects elsewhere on a push-only credential", () =>
    Effect.promise(async () => {
      ...
      try {
        const send = (credential: string) =>
          fetch(`${server.url}/private/push`, {
            ...
            headers: { "content-type": "application/json", authorization: `Bearer ${credential}` },
            body: JSON.stringify({
              url: "http://127.0.0.1:1/elsewhere",
              refs: [{ local: "main" }],
            }),
          });
  ...
  it.effect("walks the readme's quickstart: init, hub init, credential, clone", () =>
    Effect.promise(async () => {
  ...
  describe("whoami", () => {
    ...
    it.effect("says how much of a budget is left, where a repository sets one", () =>
      Effect.promise(async () => {
    ...
    it.effect("answers for a member from the private half alone", () =>
      Effect.promise(async () => {
    ...
    it.effect("names what a protected branch will judge the push by", () =>
      Effect.promise(async () => {
    ...
    it.effect("refuses a key the repository never granted, and says which key it asked about", () =>
      Effect.promise(async () => {
        await enrol("repo.read,source.push");
    ...
    it.effect("reports a revoked key as revoked rather than as a stranger", () =>
      Effect.promise(async () => {
        await enrol("repo.read,source.push");
        const answer = await whoami(path.join(root, "id_agent"));
        await cli([
    ...
    it.effect("says a repository with no genesis has no membership to grant", () =>
      Effect.promise(async () => {
        await cli(["init", "--root", root, "project"]);
        await writeKeyPair(path.join(root, "id_agent"), "agent@example.com");
  ...
  describe("the client's view of a remote", () => {
    it.effect("reports a url nothing is pinned for", () =>
      Effect.promise(async () => {
    ...
    it.effect("says when there was nothing to forget", () =>
      Effect.promise(async () => {

...
describe("Static", () => {
  ...
  it("answers the root with the page, and a built directory as built", async () => {
  ...
  it("refuses to leave the root, however the path spells it", async () => {
  ...
  it("answers nothing for a directory or a miss", async () => {
    assert.equal(await fileAt(root, "/chunks"), null);
    assert.equal(await fileAt(root, "/nope.js"), null);
  });

  it("types what it serves by extension", async () => {
    const typeOf = async (path: string): Promise<string | undefined> =>
      (await assetResponse(root, new Request(`http://host${path}`)))?.headers.get("content-type") ??
      undefined;
  ...
  it("keeps a HEAD's headers and drops its body", async () => {
  ...
  it("leaves every method that is not a read to the API", async () => {

...
describe("hub Queue", () => {
  ...
  it.effect("keeps entries in the order they were entered", () =>
    Effect.promise(async () => {
      const state = await scenario(
        Effect.gen(function* () {
          const world = yield* opened();
  ...
  it.effect("drops an entry's candidate when its head moves", () =>
    Effect.promise(async () => {
  ...
  it.effect("clears candidates on a reset and keeps the entries", () =>
    Effect.promise(async () => {
  ...
  it.effect("takes an entry out, and puts it back when it is entered again", () =>
    Effect.promise(async () => {
      const state = await scenario(
        Effect.gen(function* () {
          const world = yield* opened();
      ...
      assert.deepEqual(state.gone.left, [
        { pr: state.gone.left[0]?.pr ?? "", reason: "withdrawn" },
      ]);
      assert.equal(state.back.entries.length, 1, "leaving is undone by saying the opposite");
    }),
  );

  it.effect("ignores a candidate for something that is not queued", () =>
    Effect.promise(async () => {
      const state = await scenario(
        Effect.gen(function* () {
          const world = yield* opened();
  ...
  it.effect("refuses a record naming something that cannot be an object id", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses a record naming something that cannot be a pull request", () =>
    Effect.promise(async () => {
      const failure = await scenario(
        Effect.gen(function* () {
          const world = yield* opened();
  ...
  it.effect("ends a queue for any member holding the capability", () =>
    Effect.promise(async () => {
  ...
  it.effect("names who opened a queue, so a caller can ask before acting", () =>
    Effect.promise(async () => {
      const state = await scenario(
        Effect.gen(function* () {
          const world = yield* opened();
  ...
  it.effect("steps a closed queue aside from the branch it served", () =>
    Effect.promise(async () => {
  ...
  it.effect("admits nothing into a queue that has ended", () =>
    Effect.promise(async () => {
  ...
  it.effect("reaches the live queue without reading the ones it replaced", () =>
    Effect.promise(async () => {
  ...
  it.effect("finds a queue by the branch it serves", () =>
    Effect.promise(async () => {
      const found = await scenario(
        Effect.gen(function* () {
          const world = yield* opened("refs/heads/release");
  ...
  it.effect("names the ref a queue lives on, and reads it back", () =>
    Effect.sync(() => {
  ...
  it.effect("ignores every record nobody signed", () =>
    Effect.promise(async () => {

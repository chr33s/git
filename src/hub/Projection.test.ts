...
describe("hub projection", () => {
  it.effect("refuses an id that cannot name a ref this repository can find again", () =>
  ...
  it.effect("refuses to fold a pull request past the ceiling, and to push one there", () =>
    Effect.promise(async () => {
      ...
      assert.equal(
        outcome.refuses,
        false,
        "and the boundary refuses the push that would get there",
      );
    }),
  );

  it.effect("qualifies a base branch spelled without its refs/heads/ prefix", () =>
    Effect.promise(async () => {
  ...
  it.effect("projects an opened pull request", () =>
    Effect.promise(async () => {
      const state = await scenario(
        Effect.gen(function* () {
          const where = yield* world();
  ...
  it.effect("derives the head from the events, with no mutable head ref", () =>
    Effect.promise(async () => {
      const outcome = await scenario(
        Effect.gen(function* () {
          const repository = yield* Repository;
  ...
  describe("reviews", () => {
    it.effect("counts an approval of the current revision", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("makes an approval stale when the head moves, without unmaking it", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("makes an approval stale when the base moves, not only the head", () =>
      Effect.promise(async () => {
    ...
    it.effect("does not let a ground-down retarget rewrite what an approval was given for", () =>
      Effect.promise(async () => {
        ...
        const outcome = await scenario(
          Effect.gen(function* () {
            ...
            yield* repository.setRef({
              name: ref,
              to: yield* Event.join(pr, [approved!, sibling]),
            });
        ...
        assert.equal(
          approvals(outcome.state).length,
          0,
          "so it authorizes nothing on the new base",
        );
      }),
    );

    it.effect("resolves a thread whose id somebody else claimed later", () =>
      Effect.promise(async () => {
    ...
    it.effect("folds on when an event names a trust head this host will not walk", () =>
      Effect.promise(async () => {
    ...
    it.effect("refuses a trust head that is not an object id", () =>
      Effect.promise(async () => {
    ...
    it.effect("gives back the id of a merge it refuses", () =>
      Effect.promise(async () => {
    ...
    it.effect("refuses a merge of a revision the pull request never proposed", () =>
      Effect.promise(async () => {
    ...
    it.effect("walks a head it refuses exactly once, not once per ask", () =>
      Effect.promise(async () => {
        ...
        assert.equal(
          walks,
          2,
          "six events naming one unwalkable head is one refused walk per pass",
        );
      }),
    );

    it.effect("counts approvers, not approval events", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("does not let a comment-review cancel the same author's approval", () =>
      Effect.promise(async () => {
    ...
    it.effect("withdraws an approval with a back-dated rejection too", () =>
      Effect.promise(async () => {
    ...
    it.effect("lets a later rejection withdraw the same author's approval", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("does not count a dismissed approval", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("refuses an approval from a member who may review but not approve", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("comments", () => {
    it.effect("threads a reply and resolves it", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("reopens a resolved thread", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("checks", () => {
    it.effect("records a completed check against the revision it ran on", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("does not let a late start undo a finish for the same revision", () =>
      Effect.promise(async () => {
    ...
    it.effect("refuses a check signed by a bot trusted for a different check", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("does not count a check that ran on a superseded revision", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("lifecycle", () => {
    ...
    it.effect("closing a merged pull request does not unmerge it", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("refuses a merge event from somebody who may not merge", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("an event's declared trust head", () => {
    it.effect("is raised to one an earlier event in the same pull request named", () =>
      Effect.promise(async () => {
    ...
    it.effect("holds a retargeting pr.opened to the floor, like every other event", () =>
      Effect.promise(async () => {
    ...
    it.effect("may match what an earlier event named, which is the honest case", () =>
      Effect.promise(async () => {
    ...
    it.effect("does not drop an honest event from a replica whose trust log lags", () =>
      Effect.promise(async () => {
  ...
  describe("a forged duplicate event id", () => {
    it.effect("cannot displace the authorized event that claimed it", () =>
      Effect.promise(async () => {
  ...
  describe("an id claimed by two authors", () => {
    it.effect("cannot evict a stranger's event, whatever the commit order", () =>
      Effect.promise(async () => {
  ...
  describe("an event folded before the opening one", () => {
    it.effect("cannot re-enable self-approval by contesting the opening", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot escape the self-approval exclusion with a stale trust head", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot approve a revision it pushed itself", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot become the author by folding first", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot burn the opening's id by replaying it parentless", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot blank the base and freeze the branch behind it", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot win descent by chaining filler commits under a graft", () =>
      Effect.promise(async () => {
    ...
    it.effect("gets no authority from there being no author yet", () =>
      Effect.promise(async () => {
  ...
  describe("resolving a review thread", () => {
    it.effect("is refused to somebody who neither opened it nor may review", () =>
      Effect.promise(async () => {
    ...
    it.effect("is allowed to the member who opened it", () =>
      Effect.promise(async () => {
        const outcome = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("retargeting a pull request", () => {
    it.effect("is refused to somebody who is neither its author nor a merger", () =>
      Effect.promise(async () => {
  ...
  describe("an approval", () => {
    it.effect("does not count when it is the pull request author's own", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot be dismissed by somebody who could not have made it", () =>
      Effect.promise(async () => {
        const outcome = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("closing a pull request", () => {
    ...
    it.effect("is allowed to its own author", () =>
      Effect.promise(async () => {
        const outcome = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("is allowed to somebody who may merge it", () =>
      Effect.promise(async () => {
        const outcome = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  describe("a review dismissal", () => {
    it.effect("drops only the review its author claimed", () =>
      Effect.promise(async () => {
  ...
  describe("a tombstone", () => {
    it.effect("reaches only the event its author claimed, not every event sharing an id", () =>
      Effect.promise(async () => {
  ...
  describe("redaction", () => {
    it.effect("removes the content and keeps the event's place in the chain", () =>
      Effect.promise(async () => {
        const outcome = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("reads the same on a replica that still holds the payload", () =>
      Effect.promise(async () => {
    ...
    it.effect("decides the opening by the tombstone too, not by the bytes", () =>
      Effect.promise(async () => {
    ...
    it.effect("cannot be undone by a junk tombstone naming the tombstone", () =>
      Effect.promise(async () => {
    ...
    it.effect("does not let a stranger's tombstone decide what the first pass reads", () =>
      Effect.promise(async () => {
    ...
    it.effect("keeps deciding it the same way once the redactor has been revoked", () =>
      Effect.promise(async () => {
    ...
    it.effect("keeps honouring a tombstone after an ancestor's grant lapses", () =>
      Effect.promise(async () => {
        ...
        assert.equal(
          outcome.redacted,
          1,
          "the redaction this host may already have acted on stands",
        );
      }),
    );

    it.effect("does not let a member without hub.redact decide it either", () =>
      Effect.promise(async () => {
    ...
    it.effect("refuses a redaction from a member without hub.redact, and writes nothing", () =>
      Effect.promise(async () => {
    ...
    it.effect("refuses one pushed straight at the ref, which is what a replica sees", () =>
      Effect.promise(async () => {
    ...
    it.effect("will not redact a tombstone", () =>
      Effect.promise(async () => {
  ...
  describe("integrity", () => {
    it.effect("ignores an event written for another repository", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("ignores an unsigned event", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("refuses a revision that is not hash-qualified", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
    ...
    it.effect("keeps folding after an event it had to reject", () =>
      Effect.promise(async () => {
        const state = await scenario(
          Effect.gen(function* () {
            const where = yield* world();
  ...
  it.effect("lists the pull requests a repository holds", () =>
    Effect.promise(async () => {
...
describe("what a compare-and-swap compares against", () => {
  it.effect("appends against what the hub ref holds, not what it resolves to", () =>
    Effect.promise(async () => {
      ...
      assert.equal(
        ...
        "and the second append is what stands",
      );
    }),
  );
});

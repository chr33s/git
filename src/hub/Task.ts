...
export const TaskPayload = Schema.Union([TaskOpened, TaskClaimed, TaskReleased, TaskClosed]);
...
export const entries = Effect.fn("hub.Task.entries")(function* (task: string) {
  ...
  const events: Array<{ readonly commit: Oid; readonly payload: TaskPayload }> = [];
...
export const project = Effect.fn("hub.Task.project")(function* (task: string, now?: Date) {
  ...
  let opened: TaskPayload | null = null;
  let claim: { readonly by: Oid; readonly expiresAt: string } | null = null;
  ...
  for (const { commit, payload } of walked.events) {
    switch (payload.type) {
      case "task.opened":
        if (opened === null) opened = payload;
        break;
      case "task.claimed":
        // The earliest live claim holds: a second claimant appending after one
        // that has not expired is racing, not taking over.
        if (claim === null || Date.parse(claim.expiresAt) <= at.getTime()) {
          claim = { by: commit, expiresAt: payload.expiresAt };
        }
        break;
      case "task.released":
        claim = null;
  ...
  const live = claim !== null && Date.parse(claim.expiresAt) > at.getTime();
  return {
    task,
    exists: opened !== null,
    title: opened !== null && opened.type === "task.opened" ? opened.title : "",
    description: opened !== null && opened.type === "task.opened" ? opened.description : "",
    refs: opened !== null && opened.type === "task.opened" ? opened.refs : [],
    // `available` is the question an agent woken by a task actually asks.
    available: opened !== null && closed === null && !live,

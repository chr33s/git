...
describe("what a name may be", () => {
  it.effect("reads no object from a name that is not an object id", () =>
...
describe("packed-refs", () => {
  ...
  it.effect("lists and reads refs that live only in packed-refs", () =>
    Effect.promise(async () => {
  ...
  it.effect("prefers a loose ref over the packed entry, as git does", () =>
    Effect.promise(async () => {
      // SAFETY: forty lowercase hex characters by construction, which is exactly
      // what the Oid brand stands for.
  ...
  it.effect("re-reads packed-refs after another process rewrites it", () =>
    Effect.promise(async () => {
      // SAFETY: forty lowercase hex characters by construction, which is exactly
      // what the Oid brand stands for.
  ...
  it.effect("deletes a ref that lives only in packed-refs", () =>
    Effect.promise(async () => {
  ...
  it.effect("refuses to read a reflog outside this repository", () =>
    Effect.promise(async () => {
      const read = await withRoot(async (root) => {
  ...
  it.effect("resolves a detached HEAD to the commit it holds", () =>
    Effect.promise(async () => {
  ...
  it.effect("keeps what a packed ref reaches when gc runs", () =>
    Effect.promise(async () => {
...
describe("the ref directory", () => {
  ...
  it.effect("lists a ref whose name ends in .tmp", () =>
    Effect.promise(async () => {
  ...
  it.effect("resolves a symbolic ref rather than listing its text as an oid", () =>
    Effect.promise(async () => {
      const listed = await withRoot(async (root) => {
  ...
  it.effect("puts an atomic batch back when one of its writes fails", () =>
    Effect.promise(async () => {
  ...
  it.effect("writes no reflog for an atomic batch it rolled back", () =>
    Effect.promise(async () => {
      const state = await withRoot((root) =>
        Effect.runPromise(
          Effect.gen(function* () {
  ...
  it.effect("lists loose objects without mistaking pack files for them", () =>
    Effect.promise(async () => {
      const listed = await withRoot(async (root) => {
...
describe("alternates", () => {
  ...
  it.effect("reads through a chain of them, as git does", () =>
    Effect.promise(async () => {
  ...
  it.effect("does not list what it only borrows", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "git-alternates-"));
      try {
  ...
  it.effect("refuses to collect a repository git itself shared", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "git-alternates-"));
      try {
        const [parent, child] = [path.join(root, "parent"), path.join(root, "child")];
  ...
  it.effect("refuses to collect a lender whose borrower is linked into place", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "git-alternates-"));
      try {
  ...
  it.effect("refuses to collect a lender its borrower names through a symlink", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "git-alternates-"));
      try {
        const [parent, child] = [path.join(root, "parent"), path.join(root, "child")];
  ...
  it.effect("refuses to collect a recorded lender opened by a relative path", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "git-alternates-"));
  ...
  it.effect("sees a fork made after the store was built", () =>
    Effect.promise(async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "git-alternates-"));
      try {
        const [parent, child] = [path.join(root, "parent"), path.join(root, "child")];
...
describe("reflog bound", () => {
  it.effect("keeps a log with long messages under the cap", () =>
    Effect.promise(async () => {
      ...
      Effect.promise(async () => {
        ...
        try {
          ...
          assert.equal(
            await packDescriptors(),
            before,
            "a retire through the link released nothing",
          );
        } finally {
          await retirePacksUnder(root);
      ...
      Effect.promise(async () => {
        ...
        try {
          ...
          assert.equal(
            await packDescriptors(),
            before,
            "a dropped repository kept its descriptors",
          );
        } finally {
          await retirePacksUnder(root);
          await fs.rm(root, { force: true, recursive: true });

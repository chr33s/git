  ...
  Effect.gen(function* () {
    ...
    const writeFiles = Effect.fn("Repository.writeFiles")(function* (input: {
      ...
      for (const change of input.changes) {
        ...
        parent.delete(key);
        parent.set(key, { mode, name, oid, ...(raw === undefined ? {} : { raw }) });
        forgetDirectory(full);
      }
    ...
    return Repository.of({
      ...
      receive: Effect.fn("Repository.receive")(function* (updates, options) {
        ...
        const results = applied.map((result, index): ReceiveResult => ({
          ...
          to: result.current,
          ok: result.applied,
          // The store's own reason when it has one — a ref it could not write
          // is not a ref somebody else moved, and a client that is told the
          // latter retries a push that cannot succeed.
          ...(result.applied ? {} : { reason: result.reason ?? "ref moved" }),
        }));

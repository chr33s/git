...
describe("PktReader", () => {
  it.effect("reads a payload, the special packets, then end of input", () =>
  ...
  it.effect("reads a line split across chunk boundaries", () =>
    Effect.promise(async () => {
  ...
  it.effect("keeps every side-band packet inside git's line limit", () =>
    Effect.sync(() => {
  ...
  it.effect("refuses a length header that is not four hex digits", () =>
    Effect.promise(async () => {
  ...
  it.effect("hands the unread remainder to the pack parser", () =>
    Effect.promise(async () => {

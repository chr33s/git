...
describe("PackIndex", () => {
  it.effect("round-trips entries, sorted by oid", () =>
    Effect.sync(() => {
  ...
  it.effect("writes a cumulative fanout consistent with the oid table", () =>
    Effect.sync(() => {
  ...
  it.effect("finds every entry and reports absent oids as null", () =>
    Effect.sync(() => {
  ...
  it.effect("stores offsets past 2^31 in the large-offset table", () =>
    Effect.sync(() => {
  ...
  it.effect("rejects a corrupt trailer, bad magic and an unknown version", () =>
    Effect.sync(() => {
  ...
  it.effect("computes crc32 against known vectors", () =>
    Effect.sync(() => {
...
describe.skipIf(!hasGit)("PackIndex interop with git", () => {
  ...
  it.effect("parses a git-written .idx and agrees with git verify-pack", () =>
  ...
  it.effect("rebuilds git's .idx byte for byte from its entries", () =>
    Effect.promise(async () => {

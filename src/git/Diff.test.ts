...
describe("Diff", () => {
  it.effect("matches the longest common subsequence, not the first one it finds", () =>
    Effect.sync(() => {
  ...
  it.effect("keeps a long unchanged file entirely matched", () =>
    Effect.sync(() => {
  ...
  it.effect(
    "still matches the untouched ends of a file whose middle is past the search bound",
    () =>
      Effect.sync(() => {
  ...
  it.effect("merges edits at opposite ends of a file too different to diff in the middle", () =>
    Effect.sync(() => {
      ...
      assert.equal(
        merged.content,
        `${["head", ...middle("new"), "tail", "appended"].join("\n")}\n`,
      );
    }),
  );

  it.effect("groups a replacement and a trailing insertion into separate hunks", () =>
    Effect.sync(() => {
  ...
  it.effect("renders a unified diff with git's header, range and three lines of context", () =>
    Effect.sync(() => {
  ...
  it.effect("splits changes further apart than twice the context, and merges nearer ones", () =>
    Effect.sync(() => {
  ...
  it.effect("reports no diff at all for identical text", () =>
    Effect.sync(() => {
  ...
  it.effect("uses a zero-length old range when the file is created from nothing", () =>
    Effect.sync(() => {
      assert.equal(
  ...
  it.effect("marks a missing final newline on the side that is missing it", () =>
    Effect.sync(() => {
  ...
  it.effect("treats a trailing newline as a terminator rather than a separator", () =>
    Effect.sync(() => {
  ...
  it.effect("calls a file binary only for a NUL inside the first 8000 bytes", () =>
    Effect.sync(() => {
  ...
  it.effect("scores a renamed-and-edited file far above an unrelated one", () =>
    Effect.sync(() => {
      const original = text(numbered(10));
      const edited = original.replace("line 5", "line five");
      const unrelated = text(
        numbered(10, 100).map((line) => line.replace("line", "totally other")),
      );

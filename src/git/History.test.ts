...
describe.skipIf(!hasGit)("path history", () => {
  ...
  it.effect("simplifies a merge whose sides did not both touch the path", () =>
    Effect.promise(async () => {
      write("shared.txt", "1");
  ...
  it.effect("reports a merge that resolved the path to something neither side had", () =>
    Effect.promise(async () => {
      write("c.txt", "base");
      commit("root");
  ...
  it.effect("handles a path added, deleted and re-added", () =>
    Effect.promise(async () => {
      write("x.txt", "1");
      commit("add x");
  ...
  it.effect("finds a path inside a directory", () =>
    Effect.promise(async () => {

...
/**
 * Fetch one identity clone on its own runtime.
 *
 * Nested `Effect.provide` merges with the caller's `Repository`, so the
 * ambient one would win and the followed clone would write into the caller.
 * A fresh runtime is the isolation boundary; this is not inside an Effect
 * generator for that reason.
 */
const fetchIdentity = (input: {
  readonly directory: string;
  readonly principal: PrincipalId;
  readonly url: string;
    ...
    Effect.gen(function* () {
      ...
      if (stored === null || principalId(stored.genesis.repoId) !== input.principal) {
        return yield* new Invalid({
          field: "identity",
          reason: `${input.directory} does not hold the verified identity after synchronization`,
        });
      }

...
describe.skipIf(!hasGit)("Push, read back by the git binary", () => {
  it.effect("pushes a new branch the git binary then clones", () =>
  ...
  it.effect("sends only the new objects on a second push", () =>
    Effect.promise(async () => {
  ...
  it.effect("deletes a remote ref, without sending a pack", () =>
    Effect.promise(async () => {
...
describe("Push", () => {
  ...
  it.effect("pushes a pull request whose payload a tombstone has already removed", () =>
    Effect.promise(async () => {
      ...
      const results = await pushFrom("redacted", "tombstoned", [
  ...
  it.effect("refuses a non-fast-forward, and accepts it with force", () =>
    Effect.promise(async () => {
  ...
  it.effect("reports ok when the remote already has everything", () =>
    Effect.promise(async () => {
      ...
      assert.deepEqual(repeated.result, [
        { ref: "refs/heads/main", ok: true, reason: "up to date" },
      ]);
  ...
  it.effect("reports a delete of a ref the remote does not have", () =>
    Effect.promise(async () => {
  ...
  it.effect("pushes an oid, and fails on a local ref that does not resolve", () =>
    Effect.promise(async () => {
